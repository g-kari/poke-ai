...
2. **個別 seed 改善 ≠ ensemble 改善**: entropy_coef=0.02 で個別 v40
   seed は lab +3.6~+7.3pp 改善、 だが 3 個全部 ext した ensemble は
   lab 16.1% で **-10.6pp regression**
3. **PPO fine-tuning は seed diversity を消す**: v40 base ensemble が
   +5.7pp lift していたのに、 PPO 後の同じ 3 seed ensemble は single
   PEAK より -4.2pp 劣化。 PPO の収束力が diversity を均質化する
4. **Policy strength の不均衡で dilution**: 強い 1 policy (PPO 23.3%)
   と弱い 2 policy (base 13.2%) の logit averaging は弱い側に引きずら
   れる (= 18.6%, 全候補で最下位)
5. **Lab PEAK な policy は触らない方が良い**: PPO_v40 s100 ext は
   慎重設定 (lr 1/3 + entropy) でも -3.4pp regression
6. **3 つの ensemble 失敗パターン**: (a) features 起因 (v60 deck hash)、
   (b) training procedure 起因 (PPO 収束)、 (c) policy strength 不均衡
   起因 (dilution) — ensemble の必須条件は **同レベル strength + 実質的
   diversity の両立**
7. **Single policy submission が現状の最適戦略**: PPO_v40 seed=100
   single (lab 23.3% = 全探索 PEAK) を明日提出予定
