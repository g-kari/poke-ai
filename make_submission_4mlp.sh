#!/usr/bin/env bash
# Bundle the 4-MLP base ensemble: seed=0/2/100/200 all base (no ext).
#
# Lab winrate: 20.4% on 7-opp suite (= same as Mix v1 but PURE BASE,
# so we expect ratio 35.9 (3-MLP base level), not 24.0 (Mix v1).
# Expected LB: ~720-750.

set -euo pipefail
OUT="${1:-submission_4mlp.tar.gz}"
...
require train/mlp_policy_seed200.pt
...
cp train/mlp_policy_seed200.pt "$STAGE/train/"
