"""Bench an ensemble of v40 MLP policies (logit average) vs the 7 opponents.

Usage:
    scripts/run.sh python3 scripts/bench_v40_ensemble.py \\
        --weights train/mlp_policy_ext.pt,train/mlp_policy_seed2_ext.pt,train/mlp_policy_seed100_ext.pt \\
        --games 20
"""
...
def make_ensemble_agent(policies: list[MlpPolicy], deck: list[int]):
...
def bench(agent, opp, n: int) -> tuple[int, int, int]:
    random.seed(0)
    aw = bw = d = 0
    for _ in range(n):
        r0, _ = play(agent, opp)
...
def main() -> int:
    ...
    p.add_argument("--weights", required=True, help="Comma-separated paths to v40 .pt weights")
    ...
    print(f"\nv40 ensemble bench ({len(policies)} policies) @ {2 * args.games}g/opp")
    t0 = time.monotonic()
    total_w = total_l = 0
