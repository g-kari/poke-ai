"""Bench a v40 MLP policy weight file vs the 7 vendored opponents.

Usage:
    scripts/run.sh python3 scripts/bench_v40.py \\
        --weights train/mlp_policy_big.pt --games 20
"""
...
def make_policy_agent(policy: MlpPolicy, deck: list[int]):
...
def main() -> int:
    ...
    policy = MlpPolicy.load(str(args.weights))
    policy.eval()
    pa = make_policy_agent(policy, deck)

    print(f"V40 bench: {args.weights} @ {2 * args.games}g/opp")
    t0 = time.monotonic()
    total_w = total_l = 0
