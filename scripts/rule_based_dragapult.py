"""Dragapult ex rule-based agent — saved for benchmark comparison only.

Source: Kaggle notebook
  kiyotah/a-sample-rule-based-agent-dragapult-ex-deck
  https://www.kaggle.com/code/kiyotah/a-sample-rule-based-agent-dragapult-ex-deck

Adapted: replaced the hard-coded deck.csv lookup with a per-deck path so
the agent can be imported as a module alongside the other rule-based
agents (Mega Lucario, Iono, Mega Abomasnow) without env-var collisions.
...
# Per-agent deck path so multiple rule-based agents can coexist in tests.
_DECK_BASENAME = "deck_dragapult.csv"
file_path = os.environ.get("RULE_DECK_PATH_DRAGAPULT")
if not file_path:
