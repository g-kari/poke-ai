...
policy_s42 = MlpPolicy.load("train/mlp_policy_ppo_v40_s100_t42.pt")
...
a_s42 = make_agent(policy_s42)

mlp_w = s42_w = draws = 0
for i in range(40):
    env = make("cabt")
    if i < 20:
        env.run([a_3mlp, a_s42])
        ...
        if r == 1:
            ...
            draws += 1
    if (i + 1) % 10 == 0:
        print(f"  after {i + 1}: 3-MLP {mlp_w}, s42 {s42_w}, draws {draws}")

print(f"\nFinal (40 games): 3-MLP {mlp_w}, s42 {s42_w}, draws {draws}")
print(f"3-MLP winrate: {mlp_w / 40:.1%}")
print(f"s42 winrate: {s42_w / 40:.1%}")
