...
mlp_w = s500_w = draws = 0
for i in range(40):
    env = make("cabt")
    if i < 20:
        env.run([a_3mlp, a_s500])
        ...
        if r == 1:
            ...
            draws += 1
    if (i + 1) % 10 == 0:
        print(f"  after {i + 1}: 3-MLP {mlp_w}, s500 {s500_w}, draws {draws}")

print(f"\nFinal (40 games): 3-MLP {mlp_w}, s500 {s500_w}, draws {draws}")
