...
policy_s2026 = MlpPolicy.load("train/mlp_policy_ppo_v40_s100_t2026.pt")
...
a_s2026 = make_agent(policy_s2026)

s100_w = s2026_w = draws = 0
for i in range(40):
    env = make("cabt")
    if i < 20:
        env.run([a_s100, a_s2026])
        ...
        if r == 1:
            ...
            draws += 1
    if (i + 1) % 10 == 0:
        print(f"  after {i + 1}: s100 {s100_w}, s2026 {s2026_w}, draws {draws}")

print(f"\nFinal (40 games): s100 {s100_w}, s2026 {s2026_w}, draws {draws}")
print(f"s100 winrate: {s100_w / 40:.1%}")
print(f"s2026 winrate: {s2026_w / 40:.1%}")
