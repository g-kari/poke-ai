"""Mega Lucario ex rule-based agent — saved for benchmark comparison only.
...
Adapted: replaced the hard-coded `deck.csv` lookup with RULE_DECK_PATH env
var + falling-back search so this module can be imported as a normal
Python file from `scripts/` while still reading deck_mega_lucario.csv.

Bench result (40-game samples, 2026-06-17): rule-based + Mega Lucario beats
our linear-policy + our deck 28-12 (70%) and beats random_agent 40-0 (100%).
Our linear policy + Mega Lucario deck only manages 72.5% vs random (the
policy was trained on a different deck), so just swapping decks isn't
enough — the rule-based logic is doing real work.

Original docstring follows.

Mega Lucario ex Deck
Intermediate Level
This deck battles by strategically switching between Mega Lucario ex as the
main attacker, and Hariyama and Solrock as secondary attackers.
"""
...
# Load deck.csv; allow override via RULE_DECK_PATH for benchmarking
file_path = os.environ.get("RULE_DECK_PATH", "deck.csv")
if not os.path.exists(file_path):
    # Try alongside this file
    here = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(os.path.dirname(here), "deck_mega_lucario.csv")
if not os.path.exists(file_path):
