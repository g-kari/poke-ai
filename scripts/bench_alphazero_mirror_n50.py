"""AlphaZero mirror match: AZ-ON s500 vs AZ-OFF s500, 50 games.

Same methodology as scripts/bench_pimc_v6_mirror.py (= PIMC v6) for direct
comparison. AZ-ON uses alphazero_choose (MCTS + calibrated value head),
AZ-OFF uses raw MlpPolicy sampling.

Existing PIMC v5 (linear rollout): 51-49 tie at 100g
Existing PIMC v6 (MlpPolicy rollout): 52-48 tie at 50g

If AlphaZero shows >= 60% mirror match, default-ON 化 candidate.
"""
...
def az_on_agent(obs):
    ...
    max_c = int(sel.get("maxCount") or 0)
    # AlphaZero for MAIN single-choice with >= 2 options
