"""Vendored from Kaggle kernel:
  ...
  https://www.kaggle.com/code/romanrozen/strong-start-crustle-lucario-agent-v6-lb-860

Lucario + Hariyama + Solrock hybrid with explicit Crustle-aware routing
(uses non-ex Hariyama to bypass Crustle's 'Mysterious Rock Inn' ability).

Adapted for local benchmarking:
  - deck path read from RULE_DECK_PATH_ROMANROZEN_V6 env var with fallback
    to repo-root deck_romanrozen_v6.csv, falling back to
    /kaggle_simulations/agent/deck.csv in the Kaggle bundle layout.
  - everything else untouched (USE_SEARCH=False default, ships heuristic).
"""
...
# DECK
# ============================================================================
_DECK_BASENAME = "deck_romanrozen_v6.csv"
file_path = os.environ.get("RULE_DECK_PATH_ROMANROZEN_V6")
if not file_path:
    here = os.path.dirname(os.path.abspath(__file__))
    file_path = os.path.join(os.path.dirname(here), _DECK_BASENAME)
