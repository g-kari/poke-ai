"""PPO Phase 4 (v40 features): for 3-MLP base seed PPO fine-tuning.
...
from .mlp_policy import MlpPolicy  # noqa: E402
...
def make_ppo_agent(policy: MlpPolicy, rng: np.random.Generator, trace: list[PPOStep]):
...
def run_ppo_episode(
    policy: MlpPolicy, rng: np.random.Generator, opp_pool: list
...
def train(
    ...
    if warm_start and os.path.exists(warm_start):
        policy = MlpPolicy.load(warm_start, device=policy.device)
