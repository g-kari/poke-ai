...
## Task #113 close: V6 deck warm-start も失敗 (2026-06-19)

5000ep EXT3 から warm-start、 V6 deck で 44 分:
- recent 0.13 (前回 fresh 4000ep と同水準、 = warm-start でも収束)
- 累計 13.9% (前回 fresh と同じ!)

### V6 deck warm-start solo bench @ 30g/opp + V6 deck

  vs Mega Lucario: 9-21 (30.0%) ← cross-bench 33% と同水準
  vs Dragapult:    1-29 ( 3.3%) ← cross-bench 0% から微改善
  vs Iono:         3-27 (10.0%)
  vs Mega Aboma:   4-26 (13.3%) ← EXT3 our-deck 37% から -24pp
  vs Crustle Wall: 1-29 ( 3.3%) ← -27pp
  vs Crustle Dashi:1-29 ( 3.3%) ← 持続
  vs V6:           7-23 (23.3%)
  **overall: 26-184 (12.4%)** ← EXT3 single 20.5% から **-8.1pp 悪化**

Cross-bench (10%) から +2.4pp で warm-start の意義はあったが、 LB 換算で
**LB ~350 推定** (EXT3 LB 523 より大幅低い)。

### V60 deep-learning 路線の最終総括 (5 種試行)

| approach | overall lab | LB 換算 / 実測 |
|---|---|---|
| 3-MLP (= v40 baseline) | 23.3% | **679.6** |
| V60 EXT3 single | 19.7% (80g) | **523.1** (実測) |
| V60 EXT1+EXT3 ensemble | 18.1% | ~480 推定 |
| V60 EXT3+seed31415 ensemble | 18.6% | ~500 推定 |
| V60 EXT3 + V6 deck (no retrain) | 10.0% | ~165 推定 |
| **V60 V6deck warm-start** | **12.4%** | **~350 推定** |
| PIMC v1-v5 (heuristic) | 5-10% | n/a |

### 確定的知見

1. **3-MLP (LB 679) が我々の deep-learning best**、 V60 系すべて下位
2. V60 ensemble は features 共通で seed diversity が効かず -2pp
3. deck 切替えは 7 opp で壊滅 (-10pp 以上)
4. warm-start でも deck の shock は緩和できない (-8pp)
5. PIMC heuristic は 5-10% 天井
6. Crustle Dashi 0% は **6 試行で 6 回持続** = ex pokemon を deck から
   抜かない限り解決不可能 (= deck 構造問題)

### 次の打ち手 (V60 路線終了後)

- **PPO 実装** (large refactor): 振動制御で 25%+ を目指す中長期目標
- **3-MLP を features_v60 で再訓練**: seed 0/2/100 を v60 features で
  fresh 学習、 v40 で効いた ensemble trick を v60 で再現する試み
- **Behavioral cloning from V6 agent**: V6 agent の選択を policy で模倣、
  bench 時に V6 90% 相当を狙う (= rule-based vendor 経由でない学習)
- **submission 路線変更**: 53812882 (V60 LB 523) は撤回せず、 3-MLP を
  改めて投げて LB 679 baseline 確保

## 🎯 ストラテジー部門 (締切 9/14) report 準備 (2026-06-19)

シミュレーション部門の technical 探索が一段落、 ストラテジー部門への準備
開始。 ここまでの **30 サイクル相当の探索ログ** を整理する。

### Report の骨子案

1. **問題設定**: 不完全情報ゲームでの policy 学習
   - 観測される: 自分の手札 + 場の board state + 相手の見える pokemon
   - 観測されない: 相手の手札 / deck 残り / prize 内容
   - 1 game = 30+ ターン × 各ターン 多段 select、 reward は終局 ±1 のみ
   - = **sparse credit assignment** が core difficulty

2. **試したアプローチ** (12 系統):
   - 線形 policy (REINFORCE numpy): baseline 95% vs random
   - 3-MLP ensemble (v40 features、 seed 0/2/100): **LB 679.6** ← 学習 best
   - V60 (60-d features with opp deck-id fingerprint)
   - V60 ensemble (seed diversity 失敗)
   - V60 + V6 deck (cross-bench で deck shock 判明)
   - deck-builder agent (Task #107: v1-v9 で構造的限界)
   - GA loop (1-card swap、 8g/eval 上限ノイズ)
   - PIMC v1-v5 (heuristic、 5-10% 天井)
   - Crustle 検出 + secondary rotation agent (Task #105: 手動 logic 限界)
   - vendored rule-based: Iono (LB 762)、 CrustleDashi (888)、 V6 (926)

3. **発見した法則**:
   - **lab 1pp ≈ LB 43 ポイント** (両 deep-learning submission で実測)
   - **Crustle Dashi 0% は ex pokemon を deck から外さない限り解決不可**
     (6 試行 6 回持続)
   - **deck と policy は強く coupled**: deck.csv だけ変えても overall -10pp
   - **features 共通の policy ensemble は seed diversity が効かない** (= V60)
   - **lab → LB 変換は非線形**: 23.3% → 679 と 67.3% → 870 を結ぶ曲線

4. **未実装の方向**:
   - PPO (= sparse reward での variance 削減)
   - AlphaZero (= PIMC + 学習 value head)
   - Behavioral cloning from V6 agent (= 上位 rule-based の模倣学習)
   - Transformer features (= card-level representation)

5. **教訓・反省**:
   - 路線判断を 5-10 サイクルで切り上げる難しさ (= sunk cost fallacy)
   - lab signal が LB に translate するが、 absolute 値の予測は困難
   - heuristic value function (deck-builder、 PIMC) は noise floor 内で
     false improvement を量産する
   - **Submission build 体制の自動化が遅れた**: 53810836/53812115 ERROR の
     ように、 ローカル動作と Kaggle 動作の差を事前検出する仕組みが必須

このログは別 file (= `STRATEGY_REPORT.md`) に整理予定、 次サイクル以降で
書き出す。

## v40 baseline 改良試行 (2026-06-19、 在 background)

3-MLP の最強策 (LB 679) を **更に warm-start で学習継続**:
- seed=0 base (= mlp_policy.pt) を warm-start
- mlp_train (v40 features) で 5000ep 追加
- lr=1e-4、 seed=271828
- 期待: 3-MLP の v40 features は learnability 低そうだが、 振動が V60 ほど
  ひどくなければ +1-2pp 改善できる可能性
- 結果が出れば LB 680 → 700+ の見込み

### Task #114 close: v40 warm-ext は single で天井 (2026-06-19)

5000ep, 23 分, recent 0.59 (mirror で base に勝ち越し signal):

  vs Mega Lucario: 5-25 (16.7%)
  vs Dragapult:    7-23 (23.3%)
  vs Iono:         4-26 (13.3%)
  vs Mega Aboma:   8-22 (26.7%)
  vs Crustle Wall: 9-21 (30.0%)
  vs Crustle Dashi: 2-28 (6.7%)
  vs V6:           6-24 (20.0%)
  **overall: 41-169 (19.5%)** ← 3-MLP ensemble 23.3% に届かず

仮説検証:
- recent 0.59 は **「base policy 単独に勝つ」 が selfplay の意味**であり、
  **rule-based 強相手には改善小さい**
- single 19.5% は 3-MLP の 1 member 相当
- 4-MLP として ensemble に加えると base seed=0 と方向が似て (= warm-start
  起源)、 前回 5 連敗パターン (seed=7/11/0_iono_ft 等) の再現リスク
- archive 行きが妥当

### v40 路線の確定

- 3-MLP (seed 0/2/100) ensemble 23.3% **が天井**
- warm-start 単独で +1-2pp の改善は出るが、 ensemble member として組み込めない
- v40 features の learnability 限界 = features 改修なしでは LB 679 で固定

→ **本コンペ参加期間中の deep-learning 最大成果は 3-MLP (LB 679)** で確定。
これ以上の improvement には PPO 等の large refactor が必要、 時間予算的に
シミュレーション部門 (8/16 締切) には間に合わない。 残り時間をストラテジー
部門 (9/14 締切) report 強化に集中する判断。
