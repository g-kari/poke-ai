...
18+ 系統 (線形 / 3-MLP base / 4-MLP base / V60 single / V60 ensemble /
V6 deck / deck-builder / GA / PIMC / Crustle 検出 / vendored rule-based /
BC v1/v2 / BC+RL (BCRL1/2/3/3-ent) / Bigger MLP / mixed ensemble /
PPO v60 chain (10 variants) / PPO v40 chain) を試行し、 **LB best は
rule-based vendored の CrustleDashimaki (LB 874.7) と V6 (860.8)、
deep-learning best は 3-MLP base (seed=0/2/100, lab 18.9%, LB 679.6)。
最終探索の single PEAK は PPO_v40 seed=100 (lab 23.3% @ 700g, 期待 LB
~815、 明日 UTC で提出予定)**。
...
- **TrueSkill σ settling 中の LB は ±150-200 振れる**: 提出直後の
  数十時間で誤判断しない
- **PPO fine-tuning は seed diversity を消す**: v40 で base ensemble が
  +5.7pp lift していたのに、 PPO 後の同じ 3 seed ensemble は single
  PEAK より -4.2pp 劣化 ([[5.3l]])
...
| **我々自身の DL single PEAK (未提出)** | PPO_v40 seed=100 (lab 23.3% @ 700g, 期待 LB ~815) |
| **試行した DL 系統数** | 18+ (BC/V60/v40/ensemble/mixed/PPO v60 chain/PPO v40 chain) |
...
### 5.3l PPO_v40 chain (= 新発見、 single lab PEAK = 23.3% & ensemble lift 消失)

v60 PPO 系統 10 変種で lab 上限 19.0% (= PPO1) に張り付いた後、
**v40 features に PPO を適用**する角度を試した。 動機:
- v40 + 3-MLP ensemble は seed diversity で +5.7pp lift を実現済み
  (= 3-MLP base LB 679.6 の構造的支柱)
- v40 PPO 個別 policy が同じ +5.7pp lift パターンを再現できれば、
  3-MLP-PPO ensemble で lab 25%+ → LB 800+ の可能性

ppo_buffer.py から `features_v60` import を削除して features-agnostic
化、 `train/ppo_train_v40.py` を sed で派生 (`features_v60` →
`features`, `MlpPolicyV60` → `MlpPolicy`) して 3 seed × 1280 ep 訓練。

| seed | ベース lab | PPO 後 lab | 差分 |
|------|---------|---------|------|
| 0    | 13.2%   | 16.9%   | +3.7pp |
| 2    | 13.2%   | 15.6%   | +2.4pp |
| 100  | 18.0%   | **23.3%** | +5.3pp |

**seed=100 single lab 23.3% は我々の全探索で記録した single policy
の lab PEAK** (= 過去 best は 3-MLP base seed=100 の 18.0%, PPO1 の
19.0%)。 期待 LB は ratio 35 × 23.3% = **~815** (= 3-MLP base LB 679.6
を +136 超過)。

ところが 3-MLP-PPO ensemble (= 3 つの PPO_v40 policy を logit 平均) は:

```
vs Mega Lucario: 21.0% (single best 27.0% から -6pp)
vs Dragapult ex: 26.0% (single best 30.0% から -4pp)
vs Iono: 5.0% (single best 10.0% から -5pp)
vs Mega Abomasnow: 29.0% (single best 32.0% から -3pp)
vs Crustle Wall: 24.0% (single best 37.0% から -13pp)
vs Crustle Dashi: 7.0% (single best 7.0% で同等)
vs V6: 22.0% (single best 22.0% で同等)
overall: 134-566 (19.1%) — single seed=100 の 23.3% から **-4.2pp 劣化**
```

**仮説の崩壊**: 3-MLP base v40 で +5.7pp 効いた ensemble lift パターンが
PPO_v40 では消失。 期待: 23.3% × 1.244 ≈ 28.9% (= base ensemble の
26.7% を超える)、 実測: 19.1% (= seed=100 single より下、 seed=0/2 の
中間)。

**根本原因 (推定)**:
- **PPO fine-tuning が seed diversity を消す**: 各 seed の base policy は
  異なる initialization で異なる sub-optimal mode に収束していたが、
  PPO 1280 ep の clipped surrogate は全 seed を同じ高 reward mode に
  pull する → logit averaging が「合意」を強化するだけで decorrelation
  benefit を生まない
- **3-MLP base ensemble の lift は seed × initialization の組み合わせ
  自体に依存**: fine-tuning する前の "weak but diverse" 状態が ensemble
  の鍵だった可能性
- これは [[5.3e]] (v60 ensemble lift 棄却) と [[5.3h]] (entropy が
  ensemble を壊す) を補強する 3 つ目の事例

**含意**:
1. **Ensemble lift は feature space × training procedure に依存する非自明
   な現象**: features が v40 でも、 training が PPO になると消える
2. **Single-policy PEAK を狙うのが現状の最適戦略**: PPO_v40 seed=100 を
   そのまま提出
