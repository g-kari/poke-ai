...
### 5.3c+ BCRL2 LB 結果 (= 462.2、 lab→LB ratio 23.9)

BCRL2 (lab 19.3%) を Kaggle に提出 → LB **462.2**。 期待 (~590) を
**-130 point** 下回り、 V60 EXT3 (578.7) より低い。 学習 cost と
LB 結果の関係:

| 試行 | episodes (累積) | lab | LB | ratio |
|---|---|---|---|---|
| 3-MLP | 2000 ×3 seed | 26.7% | 679 | 25.4 |
| V60 EXT3 | 10500 | 20.5% | 578.7 | 28.2 |
| BCRL2 | BC500 + RL7000 | 19.3% | 462.2 | **23.9** |

**学び**:
- ratio が分散 (24-28) するのは、 lab bench (= meta 5 deck) が LB の
  実際の対戦相手分布をカバーしきれないため
- BCRL2 の **低 ratio** は BC prior が特定相手 (Mega Aboma 37.5%, V6
  27.5%) に過剰最適化し、 LB の多様な opponent (PIMC / MCTS / RL agent
  含む) に対し脆弱になったと推測
- **重要結論**: lab 5pp の改善 (15% → 20%) が LB ~85 point に対応
  するが、 BC prior 依存だと LB に転じない
