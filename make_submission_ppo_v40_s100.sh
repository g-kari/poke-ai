#!/usr/bin/env bash
# Bundle PPO_v40 seed=100 single — lab 23.3% on 7-opp 700g (= our highest
# single-policy lab achievement). Expected LB ~815 at ratio 35.
#
# Layout uses the standard make_submission.sh shape but with only
# mlp_policy_ppo_v40_s100.pt as the v40 weight (= single policy, not ensemble).

set -euo pipefail
OUT="${1:-submission_ppo_v40_s100.tar.gz}"
...
require train/mlp_policy_ppo_v40_s100.pt
...
cp train/mlp_policy_ppo_v40_s100.pt "$STAGE/train/"
